...
<script type="text/javascript" src="<?=$_SESSION['SWFRoot']?>swfupload.js"></script>
<script type="text/javascript" src="<?=$_SESSION['SWFRoot']?>js/fileprogress.js"></script>
<script type="text/javascript" src="<?=$_SESSION['SWFRoot']?>js/jquery.swfupload.js"></script>
...
var isMacUser = <?php echo ($_SESSION['isMacUser'] ? 'true' : 'false'); ?>;
var successURL = '<?php echo ($_SESSION['SUCCESS_REDIRECT']); ?>';  
...
$(function(){
        $('#swfupload-control').swfupload({
                upload_url: "http://<?=$_SESSION['S3_BUCKET'];?>.s3.amazonaws.com/",
                post_params: {"AWSAccessKeyId":"<?=$_SESSION['AWS_ACCESS_KEY_ID']?>", "key":"LV" + randNum + "_" + "${filename}", "acl":"public-read", "policy":"<?=$_SESSION['policyDoc64']?>", "signature":"<?=$_SESSION['sigPolicyDoc']?>","success_action_status":"201", "content-type":"image/"},
                http_success : [201],
                assume_success_timeout : <?php echo ($_SESSION['isMacUser'] ? 5 : 0); ?>,
                ...
                // Flash Settings
                flash_url : "<?=$_SESSION['SWFRoot']?>swfupload.swf",
                custom_settings : {
...
//gets the user name form the variable which stores the email
$uid = explode("@", $_SESSION["user"]);
include('inc/db.php');

$q = "SELECT id,S3KeyFinished,S3BucketFinished FROM videos WHERE format='". $_GET["format"] . "' AND classDate='" . $_GET["classdate"] . "' AND class='" . $_GET["class"] ."' AND part='" . $_GET["part"] . "'";
$res = mysql_query($q);
$row = mysql_fetch_array($res);

$qnote = mysql_query("SELECT note FROM notes WHERE videoid='" . $row["id"] . "' AND userid='" . $uid[0] ."'");
$note = mysql_fetch_array($qnote);
?>
<script>
$(document).ready(function(){
...
<?php } ?>
 ...
 <?php if($_GET["format"] != "mp3") {?>
  $('#jp_container_1').append('<textarea id="notes" placeholder="Take and save your notes here!" style="resize: none;width: 520px; height: 80px;margin-left: 20px;"><?=$note["note"];?></textarea><button class="btn btn-primary btn-large" id="updateNotes" style="margin-top: 30px;margin-left:8px;">Save</button>');
  $('#updateNotes').live('click', function() {
     var text = $('#notes').val();
     var userid = "<?=$uid[0];?>";
     var videoid = "<?=$row["id"];?>";

     $.post("sec/updateNotes.php", {userid: userid, videoid: videoid, text: text}, function(evento) {
         $('#updateNotes').text('Saved!');
         $('#updateNotes').addClass('btn-success');
     });

     setTimeout(function() {
       $('#updateNotes').text('Save')
       $('#updateNotes').removeClass('btn-success');
     }, 1000);
  });
 <? } ?>
});
...
<?php if(isset($_GET["go"]) && $_GET["go"] == "play"){?>
<script>
$(document).ready(function(){
   $("body").attr('oncontextmenu','return false;');
});
...
<?php if(isset($_SESSION["admin"]) && (isset($_GET["go"]) && $_GET["go"] == "cla")) {?>
<script>
function upName() {
...
<?php if(isset($_SESSION["admin"]) && (isset($_GET["go"]) && $_GET["go"] == "stu")) {?>
<script>
...
<?php if(isset($_SESSION["admin"]) && (isset($_GET["go"]) && $_GET["go"] == "vid")) {?>
<script>
function updateVid() {
   $("#updateBtn").live('click', function(event) {
     event.preventDefault();
...
<?php if(isset($_SESSION["admin"]) && (isset($_GET["go"]) && $_GET["go"] == "sys")) {?>
<script>
$(document).ready(function(){
...
<?php if(isset($_SESSION["admin"]) && (isset($_GET["go"]) && $_GET["go"] == "req")) {?>
<script>
$(document).ready(function(){
...
<?php if(isset($_GET["go"]) && $_GET["go"] == "fee") {?>
<script>
$(document).ready(function(){
